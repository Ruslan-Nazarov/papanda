...
async def test_default_skill_produces_no_instructions():
    """Оба скилла = plain_answerer (или skill=None) -> промпт не меняется вообще,
    старое поведение (без skill) должно быть побитово идентичным."""
    cb = ContextBuilder()
    assert await cb._render_skill_instructions(None) == ""
    assert await cb._render_skill_instructions({"speaker": "plain_answerer", "addressee": "plain_answerer"}) == ""
...
async def test_non_default_addressee_includes_hidden_phrase_markup_instruction():
    ...
    cb = ContextBuilder()
    rendered = await cb._render_skill_instructions({"speaker": "plain_answerer", "addressee": "newbie"})
...
async def test_default_addressee_does_not_include_hidden_phrase_markup_instruction():
    ...
    cb = ContextBuilder()
    rendered = await cb._render_skill_instructions({"speaker": "professor", "addressee": "plain_answerer"})
...
async def test_speaker_only_affects_tone_not_hidden_phrase_instruction():
    ...
    cb = ContextBuilder()
    rendered = await cb._render_skill_instructions({"speaker": "engineer", "addressee": "plain_answerer"})
    assert "прагматично" in rendered or "инженер" in rendered.lower()
    assert "hidden-phrase" not in rendered
