...
if __name__ == "__main__":
    ...
    # По умолчанию ВЫКЛ: hot-reload держит лишние потоки и не нужен на сервере.
    # Для локальной разработки установите RELOAD=True в .env
    reload = (os.getenv("RELOAD", "False").lower() == "true")

    url = f"http://{host}:{port}"
    print("--- Запуск papanda v0.7.5 на FastAPI ---")
    if port != start_port:
    ...
    threading.Thread(target=open_browser, args=(f"{url}/dashboard",), daemon=True).start()
