...
def process_db(path, fname):
    ...
    try:
        ...
        # -- Books --
        elif 'book' in fname and 'read' not in fname:
            for r in fetch(['books', 'book'], 'title, start_date, read'):
                d = parse_date_str(r[1])
                rd = True if str(r[2]).lower() in ['1', 'true', 't'] else False
                if not Book.query.filter_by(title=r[0]).first():
                    db.session.add(Book(title=r[0], start_date=d, read=rd))
        ...
        # -- Read Books Stats --
            ...
            for r in fetch(['readbooks', 'read_books_stats', 'read_book'], 'countdays'):
                db.session.add(ReadBook(countdays=r[0]))
