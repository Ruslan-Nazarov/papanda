...
async def language_dashboard(
    request: Request,
    db: AsyncSession = Depends(get_db),
    word_service: WordService = Depends(get_word_service),
    user: Any = Depends(check_auth_dependency),
) -> HTMLResponse:
    """Главная страница Language Dashboard."""
    # Get layout for this specific dashboard
    language_layout = await get_setting(db, "language_layout", "[]")
    show_widget_tips = await get_setting(db, "show_widget_tips", "True") == "True"

    # Fetch dictionary metrics
    ...
    active_languages = await word_service.get_active_languages()
    all_languages = await word_service.get_all_language_names()
    fully_learned_count = await word_service.get_fully_learned_count(active_languages)
    ...
    return templates.TemplateResponse(request, "language_dashboard.html", {
        "request": request,
        ...
        "lang_anchors_json": lang_anchors_json,
    })
