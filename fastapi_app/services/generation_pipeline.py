...
# дешева (~1-3k токенов), но их до 5.
# 2026-09-15: снижено с 3 до 2 — изначальные 3 попытки были рассчитаны на
# GigaChat первым в очереди (часто мазал), но теперь для скелета первыми
# идут Cerebras/Gemini (см. TASK_ROUTES["skeleton"] в llm_provider.py),
# они на практике сходятся за 1-2 попытки. Плюс есть внешний
# _MAX_PLAN_ATTEMPTS (пробует другой Шаг 1 целиком), так что запас
# устойчивости не потерян — он просто на другом уровне. В худшем случае
# это 5 стадий × 2 попытки × 2 вызова = до 20 коротких вызовов на план.
