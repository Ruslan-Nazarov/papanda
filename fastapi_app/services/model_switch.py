"""Переключатель модели в интерфейсе + запрос остатка лимита.

Пользователь выбирает предпочитаемого провайдера («Модель» в шапке). Выбор
едет заголовком X-Model-Prefer, роутер кладёт его в contextvar
ai_service._preferred_provider на время запроса. LLMRegistry._order() ставит
этого провайдера первым (фолбэк на остальных сохраняется).
"""
