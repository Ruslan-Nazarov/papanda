...
class Settings(BaseSettings):
    ...
    # Оппонент-этап-1: перед генерацией проверять, применим ли диалектический
    # метод к теме (судья_применимости.md). Если нет — конспект не строим,
    # показываем вердикт + обычный краткий ответ. Выключить, если мешает.
    APPLICABILITY_GATE_ENABLED: bool = True
